...
public class Tank {
    ...
    public Tank(TextureAtlas atlas, float x, float y) {
        ...
        projectile.texture = new TextureRegion(atlas.findRegion("bullet"));
    }
    ...
    public void update(float dt) {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.K)) {
            projectile.setup(this.position, this.angle);
            projectile.update(dt);
        }
        checkBounds();
        projectile.checkBounds(dt);
    }
    ...
    //    batch.draw(projectile.texture, position.x, position.y);
    }
}
